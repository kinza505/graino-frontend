...
const Checkout = () => {
    ...
    // --- Yeh hissa add kiya gaya hai page ko top par lane ke liye ---
    useEffect(() => {
        window.scrollTo({
            top: 0,
            left: 0,
            behavior: "smooth" // Is se smooth scroll ho kar upar jayega
        });
    }, []);
    // -----------------------------------------------------------
    ...
    const validateForm = () => {
        ...
        } else if (!formData.phone.startsWith("03") || formData.phone.length !== 11) {
            tempErrors.phone = "Phone must be in format: 03XXXXXXXXX";
        }
        if (!formData.email) {
            tempErrors.email = "Email is required.";
        } else if (!formData.email.toLowerCase().endsWith("@gmail.com")) {
            tempErrors.email = "Only @gmail.com addresses are accepted.";
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                setIsOrdered(true);
                clearCart();
                window.scrollTo({ top: 0, behavior: "smooth" }); // Order success par bhi top par le jaye
            }
        } catch (error) {
            setLoading(false);
        ...
        <div className="min-h-screen bg-[#F8FAFC] py-10 px-6 lg:px-20 text-slate-900">
            ...
            <main className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12 items-start">
                        ...
                        <section className="pt-10 border-t border-slate-100">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2">
                                    <Input label="Street Address" name="streetAddress" value={formData.streetAddress} onChange={handleChange} error={errors.streetAddress} />
                                </div>
                            </div>
                        </section>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {["cash", "bank"].map((method) => (
                                    <button
                                        ...
                                        ${formData.paymentMethod === method ? "border-amber-500 bg-amber-50" : "border-slate-100"}`}
                                    >
                                        {method === "cash" ? "Cash on Delivery" : "Bank Transfer"}
                                    </button>
                                ))}
                            </div>
                            ...
                            {formData.paymentMethod === "bank" && (
                                <div className="mt-6 p-8 rounded-3xl bg-slate-900 text-white space-y-4">
                                    ...
                                    <div className="space-y-2 text-sm font-medium">
                                        <p><span className="font-black">Account Title:</span> Adil Masood</p>
                                        <p><span className="font-black">Bank:</span> Faysal Bank</p>
                                        <p><span className="font-black">Account Number:</span> 3645301000000960</p>
                                        <p><span className="font-black">IBAN:</span> PK73FAYS3645301000000960</p>
                                    </div>
                                    <div className="border-t border-white/10 pt-4 text-amber-300 text-sm font-bold">
                                        Please send payment receipt on WhatsApp: <br /> +923 111 122 144
                                    </div>
                                </div>
                            )}
                        </section>
                ...
                <aside className="lg:col-span-4">
                    <div ref={summaryRef} className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between font-bold opacity-70">
                                <span>Subtotal</span>
                                <span>Rs. {(cartTotal || 0).toLocaleString()}</span>
                            </div>
                            ...
                            <div className="flex justify-between text-2xl font-black">
                                <span>Total</span>
                                <span>Rs. {(finalTotal || 0).toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                </aside>
...
const Input = ({ label, error, ...props }) => (
    <div className="input-field flex flex-col space-y-1">
        ...
        <input
            ...
            ${error ? "border-red-500 bg-red-50" : "border-slate-100 focus:border-amber-500"}`}
        />
